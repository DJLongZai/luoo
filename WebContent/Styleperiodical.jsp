...
<%
	String styles = (String)request.getParameter("style");
	String p = (String)request.getParameter("p");
	int p1 = Integer.parseInt(p);
	List<Journal> journals = JournalServices.getJournalALL(p1,7);
	List<Journal> journal = JournalServices.getJournalByStyle(styles,p1);
	List<Journal> journallike = JournalServices.getJournalBylikecount();
	double count = Math.ceil(JournalServices.getJournalcountByStyle(styles)/7.0);
     ...
     <jsp:include page="StyleList.jsp"></jsp:include>
                <div class="Periodocal-list">
                  		...
                  		<%for(int i=0;i<journal.size();i++){ %>
	                        <li>
	                            <div>
	                                ...
	                                <div class="Periodocal-list-div"><a href=""><%=journal.get(i).getJournalName() %> </a></div>
	                            </div>
	                        </li>
                      	<%} %>
                    ...
                    <div class="periodocal-list-btn">
                        <ul>
                            <%if(p1 > 1){ %>
					            <li class="down"><a href="<%=request.getContextPath() %>/Styleperiodical.jsp?style=<%=styles %>&p=<%=p1-1%>"><span class="icon_up">上一页</span></a></li>
				        	<%} %>
				        	<%
				        		...
				        		for(;i<=5 && i<=count;i++){ %>
				        		<%if(p1==i){ %>
					        		<li class="page"><a href="<%=request.getContextPath() %>/Styleperiodical.jsp?style=<%=styles %>&p=<%=i%>"><span class="pageB"><%=i %></span></a></li>
					        		<%}else{ %>
					        		<li class="page"><a href="<%=request.getContextPath() %>/Styleperiodical.jsp?style=<%=styles %>&p=<%=i%>"><span class="pageA"><%=i %></span></a></li>
					        		<%} %>
				        	<%} %>
				            <%if(p1==count-1){ %>
					            <li class="down"><a href="<%=request.getContextPath() %>/Styleperiodical.jsp?style=<%=styles %>&p=<%=p1+1%>"><span class="icon_down">下一页</span></a></li>
				        	<%} %>
                        </ul>
                    </div>
                </div>
